...
5. Highlight the WAD with `Nintendo Channel` or `Minna no Nintendo Channel` as its name and press + to select it. When it is selected, press A twice to install the WADs.
...
<l class="notice generic fullwidth">If you happen to get `Error -1022/Error -1029` when installing `Nintendo Channel` or "Minna no Nintendo Channel", uninstall the channel by selecting the WAD with `-` and pressing `A`. Then, re-install the WAD.</l>
