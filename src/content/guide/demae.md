...
> This guide is for installing the [Dominos](#installation-guide-dominos) Version of the Food Channel. If you want to install the Standard (Fake Ordering) version, please choose the Standard version in the patcher.
...
3. When you see the setup screen, select `1. English Translation` in order to download `Food Channel (Domino's)`.<br><br>
...
4. Next, you’ll see this screen, asking which version of Food Channel you want, so you’ll want to pick `2. Domino's (US and Canada only)`<br><br>
![Food Channel Setup](https://github.com/RiiConnect24/Wii-Guide/blob/master/images/Demae-Dominos/choose-food-channel-ver.png?raw=true)

5. After that, you can go through the rest of the process, but once you reach this step on the Patcher, please be sure to pick the correct console platform you’re installing this on, as it will matter when using the app.<br><br>
...
2. After registering your console ID, Start up the Homebrew Channel and enter yawmME (YAWM ModMii Edition).

3. In the WAD Manager menu, locate the file named `Food Channel (Domino's) (English)`.

4. Also locate `WiiLink SPD (Wii)` or `WiiLink SPD (vWii)`, depending on the console platform youre installing the Food Channel on.

5. Press + to select both of the WAD files. When they are selected, press A twice to install the WADs.
...
3. Now you are in the Address Information menu. Press the arrows to traverse screens, you will enter your data here. When you are finished, click the `Done` button and then `Demae`.<br><br>
...
If you have used the standard version of the Food Delivery Channel before, go to the<br>`Wii Options` -> `Data Management` -> `Save Data` -> `Wii`, then find the <br>Food Delivery Channel icon and click "Erase" to delete the existing save data.

### If you have previously placed an order, skip to step 4.

1. Launch the Food Channel (aka. Food Channel) from the Wii Menu
...
> Please try erasing save data for the Demae Channel, and setting it up again! Let us know in our [Discord Server](https://discord.com/invite/reqUMqxu8D) if you continue to experience issues.
